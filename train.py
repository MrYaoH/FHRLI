...
from models import FHRLI_HGNN, FHRLI_EquivSetGNN, FHRLI_EquivSetGNN_PF

import datasets
import utils

def set_up_logger(args, sys_argv=None):
    # set up running log

    runtime_id = '{}-{}-{}-{}-{}-{}-{}-{}—{}'.format(str(time.time()), args.dname, args.method, args.l_1, args.l_2, args.dropout, args.lr, args.MLP_hidden, args.n_rules)
...
def main(args, recorder):
    # torch.manual_seed(args.seed)
    # torch.cuda.manual_seed(args.seed)
    # np.random.seed(args.seed)
    # random.seed(args.seed)

    recorder.info('Command line executed: python ' + ' '.join(sys.argv))
    recorder.info('Full args parsed:')
    recorder.info(args)
    ...
    elif args.method == 'FHRLI_EDGNN_ISF':
        model = FHRLI_EquivSetGNN(data.num_features, data.num_classes, args)
    elif args.method == 'FHRLI_EDGNN':
        model = FHRLI_EquivSetGNN_PF(data.num_features, data.num_classes, data.num_nodes, args)
    else:
    ...
    for run in range(args.runs):
        ...
        for epoch in range(args.epochs):
            ...
            # fuzziness weight decay loss
            if_wd_loss, fc_wd_loss = model.fuzziness_weight_decay_loss()
            loss = loss + args.l_1 * if_wd_loss + args.l_2 * fc_wd_loss
    ...
    logger.print_statistics(recorder=recorder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', default='./processed/cora', type=str)
    parser.add_argument('--raw_data_dir', default='./raw_data/cocitation/cora', type=str)
    ...
    parser.add_argument('--All_num_layers', default=1, type=int, help='number of basic blocks')
    parser.add_argument('--MLP_num_layers', default=0, type=int, help='layer number of mlps')
    parser.add_argument('--MLP_hidden', default=256, type=int, help='hidden dimension of mlps')
    ...
    parser.add_argument('--restart_alpha', default=0.0, type=float)
    ...
    recorder = set_up_logger(args)

    lr_set = [0.001, 0.002, 0.005]
    l1_set = [0.001, 0.005, 0.01]
    dropout_set = [0.3, 0.4, 0.5]
    l2_set = [0.05, 0.01, 0.001]
    rules_set = [3, 4, 5]
    for i in lr_set:
        for j in l1_set:
            for k in dropout_set:
                for m in l2_set:
                    for n in rules_set:
                        args.lr = i
                        args.l_1 = j
