...
class Logger:
    ...
    def print_statistics(self, run=None, recorder=None):
        if run is not None:
            ...
            recorder.info(f"All runs:")
            recorder.info(f"Highest Train: {result['max_train_mean']:.2f} + {result['max_train_std']:.2f}")
            recorder.info(f"Highest Valid: {result['valid_mean']:.2f} + {result['valid_std']:.2f}")
            recorder.info(f"  Final Train: {result['train_mean']:.2f} + {result['train_std']:.2f}")
            recorder.info(f"   Final Test: {result['test_mean']:.2f} + {result['test_std']:.2f}")
